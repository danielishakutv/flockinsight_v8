...
export default async function OutreachPage() {
  const [churches, countryRows, sources, stats, campaigns] = await Promise.all([
    db
      .select({ id: church.id, name: church.name, plan: church.plan })
      .from(church)
      .orderBy(asc(church.name)),
    db.selectDistinct({ country: church.country }).from(church).orderBy(asc(church.country)),
    leadSources(),
    pipelineStats(),
    listCampaigns(30),
  ]);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Email and SMS to churches already on FlockInsight, or to leads you&rsquo;re
          still chasing.
        </p>
      </div>
